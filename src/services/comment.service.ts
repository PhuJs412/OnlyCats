...
export const createComment = async (loginUserId: string, postId: string, content: string) => {
    if (!postId || !content) throw new Error(ErrorMessage.INVALID_POST_ID_AND_CONTENT);

    try {
        const comment = await commentDal.createCommentDAL(loginUserId, postId, content);
        if (!comment || !comment.id) throw new Error(ErrorMessage.FAILED_CREATE_COMMENT);
...
export const createReply = async (loginUserId: string, postId: string, content: string, parentCommentId: string) => {
    if (!postId || !content || !parentCommentId) throw new Error(ErrorMessage.INVALID_POST_ID_AND_CONTENT_AND_COMMENT_ID);

    try {
        const reply = await commentDal.createReplyDAL(loginUserId, postId, content, parentCommentId);
        if (!reply || !reply.id) throw new Error(ErrorMessage.FAILED_CREATE_REPLY);
...
export const updateComment = async (loginUserId: string, comment: CommentUpdate, comment_id: string) => {
    ...
    if( !commentExists) throw new Error(ErrorMessage.COMMENT_NOT_FOUND);
...
const sendCommentNotification = async (loginUserId: string, postId: string, comment: Comments) => {
    ...
    if (!post) throw new Error(ErrorMessage.POST_NOT_FOUND);
    ...
    if (!recipientId) throw new Error(ErrorMessage.RECIPIENT_NOT_FOUND);
