...
export const getReplyByCommentId = async (req: Request, res: Response) => {
    try {
        const comments = await commentService.getReplyByCommentId(req.params.comment_id);
        res.status(200).json(comments);
    } catch (error) {
        res.status(500).json({ message: (error as Error).message });
    }
...
export const createComment = async (req: Request, res: Response) => {
    try {
        ...
        await commentService.createComment(user_id, post_id, content);
...
export const createReply = async (req: Request, res: Response) => {
    try {
        ...
        await commentService.createReply(user_id, post_id, content, parent_comment_id);
        res.status(200).json('ok');
    } catch (error) {
        res.status(500).json({ message: (error as Error).message });
    }
