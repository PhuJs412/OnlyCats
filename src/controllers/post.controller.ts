...
export const getAllPost = async (req: Request, res: Response) => {
    try {
        ...
        res.status(200).json(formatResponse(200, undefined, posts));
    } catch (error) {
...
export const countTotalSharedPostById = async (req: Request, res: Response) => {
    try {
        ...
        res.status(200).json(formatResponse(200, undefined, countSharedPost));
    } catch (error) {
        res.status(500).json(formatResponse(500, (error as Error).message));
    }
};
...
export const createSharedPost = async (req: AuthRequest, res: Response) => {
    try {
        ...
        res.status(200).json(formatResponse(200, SuccessfulEnums.SHARED_POST_CREATED));
    } catch (error) {
        res.status(500).json(formatResponse(500, (error as Error).message));
    }
};
